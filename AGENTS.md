...
The accepted delivery architecture uses Netlify for the fully static build (`astro build` → `dist/`), with no SSR adapter, cookies, or service worker. Committed `netlify.toml` requires `pnpm install --frozen-lockfile && pnpm verify`, publish directory `dist`, and Node 22; the committed GitHub Actions workflow requires `pnpm verify` for pull requests targeting `master` and pushes to `master`. No application or content environment variables are required. Final remote workflow runs, Netlify settings, deploy behavior, DNS, and TLS remain pending external verification. See `docs/superpowers/plans/2026-08-13-local-content-cutover-operator-runbook.md` for the cutover/retirement operator runbook and `docs/adrs/adr_08_local_content.md` for the decision.

Cloudflare Pages remains the accepted dormant fallback. It is intended to use the same gate, Node 22 runtime, and `dist` publish directory with no application or content environment variables. Final dashboard settings, remote builds, and fallback URL availability remain pending external verification. See `docs/adrs/adr_06_cloudflare_pages.md` (superseded for production traffic), `docs/adrs/adr_07_netlify.md`, and `docs/superpowers/plans/2026-07-24-netlify-primary-hosting-operator-runbook.md`.
